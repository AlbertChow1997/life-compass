...
CREATE TABLE `blog` (
    ...
    `liked`       INT             NOT NULL DEFAULT 0 COMMENT 'Like count = liked_base + real blog_like rows',
    `liked_base`  INT             NOT NULL DEFAULT 0 COMMENT 'Static baseline (e.g. seeded demo count) that real likes add on top of, never overwritten by toggling',
...
CREATE TABLE `voucher_order` (
    ...
    `verify_code` VARCHAR(8)      NOT NULL DEFAULT '' COMMENT 'Redemption code shown as a QR + text, looked up by merchants to redeem',
    `create_time` DATETIME        NOT NULL DEFAULT CURRENT_TIMESTAMP,
